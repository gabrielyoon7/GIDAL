...
const TagDataExample = [
    ...
    {
        question_id: 'AtSchool',
        type: 'button',
        question: '오늘 학교에서 나는...',
        tags: ['자습','팀플','시험','세미나','강의','과제','공부']
    },
    {
        question_id: 'AtWorkplace',
        type: 'button',
        question: '오늘 직장에서 나는...',
        tags: ['회식','미팅','칼퇴','월루','열심']
    },
...
router.post('/findTagsPerQuestion', function (req, res, next) {
    // 전체 데이터 가져오기
    Tag.find({question_id: req.body.data.question_id}).then((tags) => {
        res.json(tags)
    }).catch((err) => {
        console.log(err);
        next(err)
